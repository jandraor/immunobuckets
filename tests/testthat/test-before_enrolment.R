test_that("estimate_lambda_e works", {
  ...
  final_weight <- 1 # Enrolment on 31st Dec
  ...
  final_weight <- 0.9 # Enrolment on 25th Nov
  ...
  expect_equal(actual, expected)
  #-----------------------------------------------------------------------------
  n_e          <- 2 # Enrolment at exactly two years old.
  init_weight  <- 1 # This individual became susceptible on 1st Jan
  final_weight <- 1 # Enrolment on 31st Dec
